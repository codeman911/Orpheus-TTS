...
def process_item(item, config, tokenizer):
    ...
    try:
        ...
        waveform, sample_rate = torchaudio.load(item["audio_filepath"])
        ...
        # Encode text
        text_ids = tokenizer.encode(item["text"], add_special_tokens=True)
        ...
        print(f"Error processing {item['audio_filepath']}: {str(e)}")
        return None
...
def convert_manifest_to_dataset(manifest_path, output_dir):
    ...
    try:
        ...
        valid_items = [
            ...
            and os.path.exists(item["audio_filepath"])
        ]
