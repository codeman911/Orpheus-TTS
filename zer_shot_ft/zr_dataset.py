...
def build_speaker_utterances_map(manifest_path):
    ...
    total_utterances = sum(len(utterances) for utterances in speaker_utterances.values())
    logger.info(f"Found {total_speakers} speakers with {total_utterances} utterances")
    
    # Filter out speakers with only one utterance (can't create pairs)
    filtered_speakers = {speaker: utterances for speaker, utterances in speaker_utterances.items() 
                        if len(utterances) > 1}
    
    filtered_speakers_count = len(filtered_speakers)
    filtered_utterances = sum(len(utterances) for utterances in filtered_speakers.values())
    
    logger.info(f"After filtering: {filtered_speakers_count} speakers with {filtered_utterances} utterances")
