...
logger.info(f"LoRA configuration - Rank: {lora_rank}, Alpha: {lora_alpha}, Dropout: {lora_dropout}")
...
training_args = TrainingArguments(
    ...
    optim="adamw_bnb_8bit",  # Use 8-bit optimizer to save memory
    max_grad_norm=1.0,
...
logger.info(f"Saving LoRA adapter to {base_repo_id}/lora_adapter")
...
logger.info(f"Saving merged model to {base_repo_id}/merged")
merged_model.save_pretrained(f"./{base_repo_id}/merged")
tokenizer.save_pretrained(f"./{base_repo_id}/merged")

logger.info("Training completed successfully!")
logger.info(f"LoRA adapter saved to: {base_repo_id}/lora_adapter")
logger.info(f"Merged model saved to: {base_repo_id}/merged")