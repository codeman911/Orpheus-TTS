...
def custom_data_collator(features):
    """
    Custom data collator that handles variable length sequences by padding and truncation.
    Matches the implementation in the pretrain directory with added memory optimizations.
    """
    input_ids = [f["input_ids"] for f in features]
    
    # Truncate sequences that are too long to prevent OOM
    input_ids = [ids[:max_length] for ids in input_ids]
    
    # Handle missing attention_mask (like in pretrain implementation)
    if any("attention_mask" not in f for f in features):
        attention_mask = [[1]*len(ids) for ids in input_ids]
    else:
        attention_mask = [f["attention_mask"] for f in features]
        # Also truncate attention masks
        attention_mask = [mask[:max_length] for mask in attention_mask]
    
    # Handle missing labels (like in pretrain implementation)
    if any("labels" not in f for f in features):
        labels = input_ids.copy()
    else:
        labels = [f["labels"] for f in features]
        # Also truncate labels
    ...
    labels = torch.nn.utils.rnn.pad_sequence(
        ...
        batch_first=True,
        padding_value=-100  # Use -100 for padding in labels (ignored by loss function)
    )
...
trainer = SFTTrainer(
    ...
    tokenizer = tokenizer,
    args = training_args,
    packing = True,  # Enable sequence packing for better efficiency
)
